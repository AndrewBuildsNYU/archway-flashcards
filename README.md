...
generation** â€” asking a model for strict JSON through the gateway, and parsing the answer defensively
...
with every request your browser makes â€” anyone who can see your screen or your devtools can read it. Give
...
Then open `index.html`. There is no build step, no package manager, and no dependencies â€” three files and
two shared assets.
...
1. **The prompt asks for one shape and nothing else**: `{"cards":[{"front":â€¦,"back":â€¦,"tag":â€¦}]}`, with
   explicit rules â€” a front that is a real question rather than a fill-in-the-blank stub, a back that
...
2. **The parse assumes the model misbehaves.** `extractJson()` strips a leading ` ```json ` fence, takes
   ...
   verbatim instead of the app throwing â€” a failed generation should still teach you what went wrong.
...
Model output is written with `textContent` and never `innerHTML` â€” a flashcard is untrusted text.
